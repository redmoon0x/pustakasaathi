    ...
    if 0 <= book_id < len(book_list):
        ...
        try:
            ...
            if len(pdf_files) > 0:
                ...
                query.message.reply_document(file_url, caption=book_title)
            else:
                query.message.reply_text("No PDF files found for the selected book.")
        except Exception as e:
            logging.error("Error occurred during book retrieval: " + str(e))
            query.message.reply_text("Failed to retrieve the book. Please try again later.")
    else:
        query.answer("Invalid book selection. Please try again.")
